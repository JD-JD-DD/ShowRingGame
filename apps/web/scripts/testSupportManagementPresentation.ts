...
assert.match(affordances, /PayPal may show the new monthly amount and an “Agree &amp; Subscribe” button/);
assert.match(affordances, /You are not starting a second subscription or paying the lower amount today/);
assert.match(affordances, /Starting change\.\.\./);
assert.doesNotMatch(affordances, /Starting upgrade\.\.\./);
