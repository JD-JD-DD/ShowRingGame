...
assert.match(workspace, /Leave a price blank to skip that dog\./);
...
assert.match(workspace, /const price = pricesByDogId\[dog\.dogId\] \?\? "";/);
assert.match(workspace, /return price !== "" && !isValidWholeDollarSalePrice\(price\);/);
assert.match(workspace, /const pricedEligibleDogs = useMemo/);
assert.match(workspace, /eligibleDogs\.filter\(\(dog\) =>\s*isValidWholeDollarSalePrice\(pricesByDogId\[dog\.dogId\] \?\? ""\)/);
assert.match(workspace, /pricedEligibleDogs\.length > 0 && invalidPriceDogIds\.size === 0/);
assert.match(workspace, /updates: pricedEligibleDogs\.map/);
assert.match(workspace, /invalidPrice && !loading/);
...
assert.doesNotMatch(workspace, /\/api\/dogs\/\[dogId\]\/list-for-sale|listDogForSale|DogListing|marketState|Include\/Exclude|includeDog|excludeDog/);
