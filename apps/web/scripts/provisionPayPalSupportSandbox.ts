...
function plan(tier: "bronze" | "silver" | "gold", name: string, amount: string, productId: string) {
  return client.createBillingPlan({
    product_id: productId,
    name,
    description: "Voluntary monthly support for ShowRing Game",
    billing_cycles: [{ tenure_type: "REGULAR", sequence: 1, total_cycles: 0, frequency: { interval_unit: "MONTH", interval_count: 1 }, pricing_scheme: { fixed_price: { value: amount, currency_code: "USD" } } }],
    payment_preferences: { auto_bill_outstanding: true, setup_fee: { value: "0", currency_code: "USD" }, setup_fee_failure_action: "CONTINUE", payment_failure_threshold: 0 },
    taxes: { percentage: "0", inclusive: false },
  }, `${requestPrefix}-plan-${tier}`);
}
...
function isVerifiedMonthlyPlan(value: Record<string, unknown>, productId: string, name: string, amount: string): boolean {
  ...
  return value.product_id === productId && value.name === name && value.status === "ACTIVE"
    && frequency?.interval_unit === "MONTH" && frequency?.interval_count === 1
    && price?.value === amount && price?.currency_code === "USD";
}
...
try {
  ...
  if (verifiedProduct.name !== "ShowRing Support" || verifiedProduct.type !== "SERVICE"
    || !isVerifiedMonthlyPlan(verifiedPlans[0], productId, "Bronze Supporter", "2.00")
    || !isVerifiedMonthlyPlan(verifiedPlans[1], productId, "Silver Supporter", "5.00")
    || !isVerifiedMonthlyPlan(verifiedPlans[2], productId, "Gold Supporter", "10.00")) throw new Error("PayPal sandbox catalog verification failed.");
  console.log(`PAYPAL_SANDBOX_PRODUCT_ID=${productId}`);
  console.log(`PAYPAL_SANDBOX_BRONZE_PLAN_ID=${ids.bronze}`);
  console.log(`PAYPAL_SANDBOX_SILVER_PLAN_ID=${ids.silver}`);
  console.log(`PAYPAL_SANDBOX_GOLD_PLAN_ID=${ids.gold}`);
} catch (error) {
  console.error(error instanceof PayPalSupportError ? error.message : error instanceof Error ? error.message : "PayPal sandbox provisioning failed.");
  process.exitCode = 1;
}
}
