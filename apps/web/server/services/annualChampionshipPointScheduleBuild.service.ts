...
export type AnnualChampionshipPointScheduleBuildResult = {
  ...
  resolutionCounts: Record<"LOCAL" | "PRIOR_PUBLISHED_SCHEDULE" | "NATIONAL_SAME_BREED_SAME_SEX", number>;
...
function scheduleDataFromResolution(args: {
  ...
  if (args.resolution.resolutionType === "LOCAL" || args.resolution.resolutionType === "NATIONAL_SAME_BREED_SAME_SEX") {
...
function matchesSchedule(existing: Record<string, unknown>, expected: Record<string, unknown>): boolean {
  return Object.entries(expected).every(([field, value]) => String(existing[field] ?? "") === String(value ?? ""));
}
...
export async function ensureAnnualChampionshipPointSchedulesForEffectiveYear(args: {
  ...
  const emptyCounts = { LOCAL: 0, PRIOR_PUBLISHED_SCHEDULE: 0, NATIONAL_SAME_BREED_SAME_SEX: 0 };
