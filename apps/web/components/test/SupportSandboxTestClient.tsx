...
type CurrentSubscription = {
  ...
  status: "PENDING" | "ACTIVE" | "PAYMENT_RETRY" | "CANCELLATION_SCHEDULED";
...
type Props = { kennelName: string; kennelSlug: string; currentSubscription: CurrentSubscription | null };
type SubscriptionResult = { providerSubscriptionId?: string; status?: string; approvalUrl?: string | null };

function supportLevel(tier: CurrentSubscription["currentTier"]) {
  return `${tier.charAt(0)}${tier.slice(1).toLowerCase()} Supporter`;
}
...
export default function SupportSandboxTestClient({ kennelName, kennelSlug, currentSubscription }: Props) {
  const [confirming, setConfirming] = useState(false);
  const [creating, setCreating] = useState(false);
  const [error, setError] = useState("");
  const [subscription, setSubscription] = useState<SubscriptionResult | null>(null);
  const [clearing, setClearing] = useState(false); const [clearConfirming, setClearConfirming] = useState(false);

  async function clearPending() { setClearing(true); setError(""); try { const response = await fetch("/api/test/support-sandbox/clear-pending", { method: "POST" }); const data = await response.json(); if (!response.ok) { setError(data.error ?? "The pending PayPal sandbox subscription could not be cleared. No ShowRing subscription data was changed."); setClearing(false); return; } window.location.reload(); } catch { setError("The pending PayPal sandbox subscription could not be cleared. No ShowRing subscription data was changed."); setClearing(false); } }
  ...
  return <section className="theme-card rounded-2xl p-5">
    ...
    {currentSubscription ? <div className="theme-status-success mt-5 rounded-2xl px-4 py-3 text-sm">
      <p className="font-semibold">{supportLevel(currentSubscription.currentTier)} sandbox subscription is {currentSubscription.status} in ShowRing.</p>
      <dl className="mt-3 space-y-1">
        <div><dt className="inline font-semibold">Support level: </dt><dd className="inline">{supportLevel(currentSubscription.currentTier)}</dd></div>
        <div><dt className="inline font-semibold">ShowRing status: </dt><dd className="inline">{currentSubscription.status}</dd></div>
        <div><dt className="inline font-semibold">Provider: </dt><dd className="inline">PayPal Sandbox</dd></div>
        <div><dt className="inline font-semibold">Provider subscription ID: </dt><dd className="inline">{currentSubscription.providerSubscriptionId}</dd></div>
        <div><dt className="inline font-semibold">Created: </dt><dd className="inline">{formatTimestamp(currentSubscription.createdAt)}</dd></div>
        <div><dt className="inline font-semibold">Updated: </dt><dd className="inline">{formatTimestamp(currentSubscription.updatedAt)}</dd></div>
        {currentSubscription.endedAt ? <div><dt className="inline font-semibold">Ended: </dt><dd className="inline">{formatTimestamp(currentSubscription.endedAt)}</dd></div> : null}
        {currentSubscription.firstSupportedAt ? <div><dt className="inline font-semibold">First supported: </dt><dd className="inline">{formatTimestamp(currentSubscription.firstSupportedAt)}</dd></div> : null}
      </dl>
      {currentSubscription.status === "PENDING" ? <div className="theme-status-warning mt-4 rounded-2xl px-4 py-3"><p>This clears the current unapproved PayPal sandbox subscription from ShowRing testing so you can start the Bronze approval flow again. The unapproved PayPal sandbox record may remain in PayPal test history. It cannot clear an active or paid subscription.</p>{clearConfirming ? <div className="mt-3"><p>Clear this unapproved sandbox subscription and allow a new Bronze test?</p><div className="mt-3 flex gap-3"><button type="button" className="theme-secondary-button rounded-xl px-4 py-2 text-sm font-semibold" disabled={clearing} onClick={() => setClearConfirming(false)}>Cancel</button><button type="button" className="theme-primary-button rounded-xl px-4 py-2 text-sm font-semibold" disabled={clearing} onClick={clearPending}>{clearing ? "Clearing pending test..." : "Clear Pending Test"}</button></div></div> : <button type="button" className="theme-primary-button mt-3 rounded-xl px-4 py-2 text-sm font-semibold" onClick={() => setClearConfirming(true)}>Clear Pending Sandbox Test Subscription</button>}</div> : null}
    </div> : subscription ? <div className="theme-status-success mt-5 rounded-2xl px-4 py-3 text-sm">
