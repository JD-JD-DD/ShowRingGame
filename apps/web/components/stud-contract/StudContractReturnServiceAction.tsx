...
export function StudContractReturnServiceAction(props: { returnServiceId: string; expiresAt: string }) {
  ...
  return <div className="mt-4 border-t border-white/10 pt-4"><p className="theme-copy text-sm">Available until {expiresAt} (real time). The same sire and dam under the original contracting kennels are required. This does not reserve the sire or bypass Stud Recovery/current eligibility, and temporary unavailability does not extend the deadline. One successful return breeding consumes this service; original stud compensation is not charged again, though normal breeding costs still apply.</p><button type="button" onClick={attempt} disabled={pending} className="theme-primary-button mt-4 rounded-xl px-4 py-3 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-60">{pending ? "Attempting Return Service…" : "Attempt Return Service"}</button>{feedback ? <p className={`mt-3 rounded-xl p-3 text-sm ${feedback.kind === "success" ? "theme-status-info" : "theme-status-danger"}`} role="status" aria-live="polite">{feedback.message}</p> : null}</div>;
}
