...
export default function BreedArtFundingBoardClient({ campaigns }: { campaigns: ArtCampaignReadDto[] }) {
  ...
  const hasActiveFilters = filters.groupName !== "ALL" || filters.breedName !== "" || filters.status !== "ALL";
    ...
    <section className="theme-card mt-5 rounded-2xl p-4 sm:p-5" aria-label="Breed artwork filters">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <label className="theme-label grid gap-1 text-sm" htmlFor="breed-art-group-filter">Group
          <select id="breed-art-group-filter" value={filters.groupName} onChange={(event) => setFilters((current) => ({ ...current, groupName: event.target.value }))} className="theme-input min-w-0 rounded-xl px-3 py-2 text-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">
        ...
        <label className="theme-label grid gap-1 text-sm" htmlFor="breed-art-name-filter">Breed Name
          <input id="breed-art-name-filter" type="search" value={filters.breedName} onChange={(event) => setFilters((current) => ({ ...current, breedName: event.target.value }))} className="theme-input min-w-0 rounded-xl px-3 py-2 text-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" />
        </label>
        <label className="theme-label grid gap-1 text-sm" htmlFor="breed-art-status-filter">Funding Status
          <select id="breed-art-status-filter" value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value === "NEEDS_FUNDING" || event.target.value === "FUNDED" || event.target.value === "DRAWING_COMPLETE" ? event.target.value as ArtCampaignStatusValue : "ALL" }))} className="theme-input min-w-0 rounded-xl px-3 py-2 text-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">
