...
import { listDogForOutsideGrooming } from "@/server/services/grooming.service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!dogId) {
      throw new Error("Choose a dog to offer for grooming.");
    }

    const result = await listDogForOutsideGrooming({
      kennelId: kennel.id,
      dogId,
    ...
    console.error("POST /api/services/grooming/list failed:", error);
    const message =
      error instanceof Error
        ? error.message
        : "Unable to offer dog for grooming.";
