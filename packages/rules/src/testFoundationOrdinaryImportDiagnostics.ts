...
const standardDeviation = (values: number[]) => { const mean = values.reduce((sum, value) => sum + value, 0) / values.length; return Math.sqrt(values.reduce((sum, value) => sum + (value - mean) ** 2, 0) / values.length); };
...
const tightMature = report("TIGHT_MATURE", context({ center: 10, variance: 1, belowShare: .5, aboveShare: .5 }));
const skewedAbove = report("SKEWED_ABOVE", context({ center: 11.2, variance: 1, belowShare: .15, aboveShare: .85 }));
const skewedBelow = report("SKEWED_BELOW", context({ center: 8.8, variance: 1, belowShare: .85, aboveShare: .15 }));
const broad = report("BROAD", context({ center: 10, variance: 9, belowShare: .5, aboveShare: .5 }));
